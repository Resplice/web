<script lang="ts">
	...
	function formatTime() {
		...
		const endDatetime = addSeconds(moment.startDatetime, moment.duration)
		const distanceFormatted = formatDistance(moment.startDatetime, endDatetime)
		return `${startFormatted} • ${distanceFormatted}`
	}
</script>

<div class="flex-1 overflow-hidden bg-brand-primary bg-opacity-20 rounded-md p-4 border-slate-600">
